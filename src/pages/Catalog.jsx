  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className="w-full h-10 pl-9 pr-3 rounded-md border border-slate-300 bg-white focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none"
      />
    </div>
  );
}
...
export default function Catalog() {
  ...
  const filtered = useMemo(() => {
    ...
    // поиск по заголовку/производителю
    if (q.trim()) {
  ...
  return (
    <main className="pt-28 bg-[var(--page-bg)]">
      <section className="container-max px-4 md:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px,1fr] gap-8">
          ...
          <aside className="rounded-xl border border-slate-200 bg-white p-4 h-fit">
            ...
            <Section title="Тип фильтра" defaultOpen>
              <Check label={TYPE_LABELS.fuel} checked={typeFuel} onChange={setTypeFuel} />
              <Check label={TYPE_LABELS.oil} checked={typeOil} onChange={setTypeOil} />
              <Check label={TYPE_LABELS.air} checked={typeAir} onChange={setTypeAir} />
              <Check label={TYPE_LABELS.pump} checked={typePump} onChange={setTypePump} />
            </Section>
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <button
                ...
                title="Фильтры применяются автоматически"
              >
                Применить
              </button>
            </div>
